...
def generate_resume_file(z_height, gcode_file, print_temp, bed_temp):
    ...
    with open(resume_gcode_path, 'w') as out_f:
        ...
        bed_temp_match = re.search(r'material_bed_temperature\s*=\s*(\d*\.?\d+)', metadata_content)
        
        if bed_temp_match:
            meta_bed_temp = bed_temp_match.group(1)
            out_f.write(f"M140 S{meta_bed_temp} ; Set Bed Temp (from metadata)\n")
        else:
            # Fallback to the saved variable ($4) if metadata parsing failed (this is robust)
        ...
        out_f.write("G1 Z5 ; Move Z up 5mm\n")
        ...
        out_f.write("G1 Z-5 ; Move Z back down 5mm\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument("bed_temp", type=float, help="The saved bed target temperature (bed_temp).")
    
    # We accept the 5th parameter (thumbnail) but it's not used in the core logic
    parser.add_argument("thumbnail", type=str, nargs='?', default="", help="The saved thumbnail base name (not used in core script logic).")

    try:
        args = parser.parse_args()
        generate_resume_file(args.z_height, args.gcode_file, args.print_temp, args.bed_temp)
